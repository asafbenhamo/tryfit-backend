...
<script>
let T="";const A=location.origin;
async function api(m,p,b){const o={method:m,headers:{"Authorization":"Bearer "+T,"Content-Type":"application/json"}};if(b)o.body=JSON.stringify(b);return(await fetch(A+"/admin"+p,o)).json()}
async function login(){T=document.getElementById("pw").value;try{const d=await api("GET","/api/stores");if(d.error){msg("lm",d.error,1);return}document.getElementById("L").style.display="none";document.getElementById("D").style.display="block";load()}catch(e){msg("lm","Failed",1)}}
async function load(){const d=await api("GET","/api/stores");if(d.error)return;const s=d.stores;let tc=0,tu=0,ac=0;s.forEach(x=>{tc+=x.credits;tu+=x.total_used;if(x.credits>0)ac++});
...
let h="";s.forEach(x=>{const b=x.credits>500?"b-high":x.credits>100?"b-mid":x.credits>0?"b-low":"b-zero";
h+='<tr><td>'+x.shop+'</td><td><span class="badge '+b+'">'+x.credits.toLocaleString()+'</span></td><td>'+x.total_used.toLocaleString()+'</td><td>'+x.usage_30d+'</td><td>'+x.plan+'</td><td><button class="btn btn-add" onclick="qAdd(\''+x.shop+'\')">+</button> <button class="btn btn-del" onclick="qDel(\''+x.shop+'\')">X</button></td></tr>'});
document.getElementById("tb").innerHTML=h||'<tr><td colspan="6" style="text-align:center;color:#aaa">No stores</td></tr>'}
async function addStore(){const s=document.getElementById("ns").value.trim(),c=parseInt(document.getElementById("nc").value)||0,p=document.getElementById("np").value;if(!s){msg("am","Enter store",1);return}const d=await api("POST","/api/stores",{shop:s,credits:c,plan:p});msg("am",d.message||d.error,!!d.error);load()}
async function doAdd(){const s=document.getElementById("as").value.trim(),a=parseInt(document.getElementById("aa").value)||0;if(!s||!a){msg("qm","Enter store & amount",1);return}const d=await api("POST","/api/credits/add",{shop:s,amount:a});msg("qm",d.message||d.error,!!d.error);load()}
async function doSet(){const s=document.getElementById("as").value.trim(),a=parseInt(document.getElementById("aa").value)||0;if(!s){msg("qm","Enter store",1);return}const d=await api("POST","/api/credits/set",{shop:s,credits:a});msg("qm",d.message||d.error,!!d.error);load()}
function qAdd(s){const a=prompt("Credits to add to "+s+"?","1000");if(a){document.getElementById("as").value=s;document.getElementById("aa").value=a;doAdd()}}
async function qDel(s){if(!confirm("Remove "+s+"?"))return;const d=await api("DELETE","/api/stores/"+encodeURIComponent(s));msg("qm",d.message||d.error,!!d.error);load()}
function msg(id,t,e){const el=document.getElementById(id);el.textContent=t;el.className="msg "+(e?"msg-err":"msg-ok");el.style.display="block";setTimeout(()=>el.style.display="none",3000)}
</script></body></html>`);
});
