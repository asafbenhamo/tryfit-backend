...
- **עקביות בין ערוצים:** כשאתה שולח את אותה הצעה לאותה לקוחה בכמה ערוצים (SMS + וואטסאפ + מייל) - השתמש בדיוק באותו נוסח הודעה ובאותו קוד קופון בכל הערוצים. אסור שהלקוחה תקבל שתי הודעות שונות על אותה הצעה - זה מבלבל ולא מקצועי.
...
const TOOL_DEFINITIONS = [
  ...
  {
    name: "sendSms",
    description: "שולח SMS בודד למספר ספציפי דרך TextMe. המערכת מוסיפה אוטומטית בסוף ההודעה קישור רכישה מעוקב - אל תכתוב 'בקישור' או 'לחצי כאן' בגוף ההודעה ואל תוסיף קישור בעצמך. חובה: לפני שליחה ללקוחה קיימת, בדוק קודם מה היא קנתה (getCustomerProfile או getRFMSegments) וכתוב הודעה אישית באמת שמזכירה את המוצר שקנתה ('ראינו שאהבת את X'). הודעה גנרית בלי הקשר אישי = הודעה גרועה. לקופון אמיתי ציין coupon_percentage - המערכת תיצור קוד פעיל בשופיפיי. אל תמציא קוד בעצמך! ההודעה מכבדת את רשימת ההסרה אוטומטית.",
    input_schema: {
      type: "object",
      properties: {
        phone: { type: "string", description: "מספר הטלפון לשליחה (05XXXXXXXX)" },
        message: { type: "string", description: "תוכן ה-SMS - אישי, מזכיר מה הלקוחה קנתה, 3-4 שורות, בלי קישורים (נוסף אוטומטית). אם יש קופון שים {COUPON} או שהקוד יתווסף בסוף" },
