...
- מייל הוא רק גיבוי ללקוחות שאין להן טלפון בכלל.
- כרגע מערכת המייל עדיין לא פעילה, אז עדיף מאוד להתמקד בלקוחות עם טלפון. אם בסגמנט יש לקוחות בלי טלפון, אפשר לציין שהן יקבלו מייל כשהמערכת תהיה מוכנה, אבל אל תבטיח שמייל נשלח עכשיו.
...
קמפיין המוני (batch):
כשבעל החנות מבקש לפנות לקבוצת לקוחות - הפעל קמפיין batch: זהה את הסגמנט (עד 50 לקוחות), הכן תבנית הודעה אחת מותאמת לסגמנט, וכל לקוחה תקבל אוטומטית קופון אישי משלה.
...
- **כשמבקשים "עוד לקוחות אחרות" / "רשימה חדשה" / "50 אחרות שלא היו ברשימה הקודמת":** קרא לכלי שליפת הלקוחות עם הפרמטר excludeContacted=true. זה מסנן אוטומטית את כל מי שכבר פנינו אליה ב-4 הימים האחרונים ומחזיר קבוצה חדשה לגמרי. אל תחזיר את אותן לקוחות שכבר הצגת. אם אחרי הסינון אין מספיק לקוחות, אמור זאת בכנות (למשל "נשארו רק 12 לקוחות חדשות שעומדות בתנאי") - אל תמציא ואל תחזור על קודמות.
...
const TOOL_DEFINITIONS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        excludeContacted: { type: "boolean", description: "אם true, מסנן לקוחות שכבר פנינו אליהן ב-4 הימים האחרונים. השתמש בזה כשמבקשים 'עוד לקוחות אחרות' / 'רשימה חדשה' כדי לא לחזור על אותן לקוחות." }
      }
    }
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        excludeContacted: { type: "boolean", description: "אם true, מסנן לקוחות שכבר פנינו אליהן ב-4 הימים האחרונים. השתמש בזה כשמבקשים 'עוד לקוחות אחרות' / 'רשימה חדשה'." }
      }
    }
  },
  {
